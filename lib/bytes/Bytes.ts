...
export class Byt3s extends Uint8Array {
    ...
    public readBytes(amount : number) {
    ...
    public escapeReadBytes(amount: number){
        let bytes = new Uint8Array(amount);

        for (let i=0; i < amount; i++){
            bytes[i] = this.escapeReadByte();
        }
    ...
    public escapeReadUint64LE(offset?: number) {
        const bytes = new Uint8Array([
            this._getEscapedValue(offset),
            this._getEscapedValue(),
            this._getEscapedValue(),
            this._getEscapedValue(),
            this._getEscapedValue(),
            this._getEscapedValue(),
    ...
    public escapeWriteUint64LE(value: number, offset?: number){
        const buff = new ArrayBuffer(8);
        const floatView = new Float64Array(buff).set([value]);
        const bytes = new Uint8Array(buff);
        this.escapeWriteBytes(bytes);
    }
    ...
    public escapeWriteBytes(bytes : Uint8Array) : void {
        for (const byte of bytes){
            this.escapeWriteByte(byte);
        }
    }

    public escapeWriteUint16LE(value : number) : void {
        const int1 = (value & 0xff);
        const int2 = (value >>> 8 & 0xFF);
    ...
    public escapeWriteUint32LE(value : number) : void {
        const int1 = (value & 0xff);
        const int2 = (value >>> 8 & 0xff);
        const int3 = (value >>> 16 & 0xff);
        const int4 = (value >>> 24 & 0xff);
